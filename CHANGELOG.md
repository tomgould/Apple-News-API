...
### Added
- `Article::addTextStyle()` method for defining reusable inline text styles (`ArticleDocument.textStyles`), separate from `componentTextStyles`

### Fixed
- JSON serialization of empty arrays in component layouts and component styles now correctly outputs `{}` instead of `[]` as required by the Apple News API
- `AppleNewsException` key path handling when passed as an array
